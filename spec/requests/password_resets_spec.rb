...
RSpec.describe "PasswordResets", type: :request do
  ...
  describe "GET /password_resets/:id/edit" do
    ...
    it "is valid information" do
      ...
      get edit_password_reset_path(user.reset_token, email: user.email)
      expect(flash[:danger]).to be_falsey
      expect(request.fullpath).to eq "/password_reset/#{user.reset_token}/edit?#{CGI.escape(user.email)}"
    end
  end

  describe "PATCH /password_resets/:id" do
    it "is invalid password" do
      post passsword_resets_path, params: { password_reset: { email: user.email } }
      user = assigns(:user)
      get edit_password_reset_path(user.reset_token, email: user.email)
